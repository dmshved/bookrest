...
app.MapDelete("users/{id}/refresh-tokens", async (string id, RevokeRefreshTokens useCase) =>
{
    bool success = await useCase.Handle(id);
    return success ? Results.NoContent() : Results.BadRequest();
}).RequireAuthorization();

app.MapPost("users/refresh-token", async (LoginUserWithRefreshToken.Request request, LoginUserWithRefreshToken useCase) =>
{
...
app.MapPost("/register", async (RegisterUser.Request request, UserManager<AppUser> userManager, RegisterUser registerUser) =>
{
...
app.MapPost("/login", async (LoginUser.Request request, UserManager<AppUser> userManager, LoginUser loginUser) =>
{
